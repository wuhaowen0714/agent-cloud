...
- **proto**(`protos/agent_cloud/v1/worker.proto`):`TurnEvent` oneof 增第 6 支 `ToolCallProgress tool_call_progress = 6`(字段同上);`scripts/gen_protos.sh` 重新生成。纯增量字段,新旧端互不影响。
...
- **前端**:
  ...
  - `ChatView` feed 分发加一支。`reset` / `error` 已整组清 blocks,pending 卡自然消失。
