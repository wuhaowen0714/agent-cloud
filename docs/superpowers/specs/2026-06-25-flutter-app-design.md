...
- 离线/弱网处理(发送失败重试、断流自动重连续接,复用后端 resume)
...
| 弱网/生命周期 | connectivity_plus(网络状态)+ WidgetsBindingObserver(前后台):断流自动重连 |
...
- 回合流:`POST /sessions/{id}/turn/stream`(发起,SSE);**`GET` 同 URL = resume 续看**(204=没在跑)
- 模型:`GET /models`
- 自动更新:`GET /app/version`(无需登录)+ APK 静态文件(nginx 托管)
- baseURL:上架 `https://app.sophclaw.icu:18080/api`;开发期可指本地/st-e

## 离线/弱网处理(对标 web,复用后端 resume)

后端回合是**有状态**的(在 Hub 的 ActiveTurn 里跑),客户端断了服务端不停——这是断流重连的基础。前端 ChatView 已用这套,App 原样对标。

### 端点语义
同一个 URL `/sessions/{id}/turn/stream`:
- **POST**(body `{content, images}`)= 发起新回合
- **GET** = resume 续看进行中的回合:`204` → 没在跑(刷历史拿结果);否则**补播**已产生事件 + 实时续接

### 断流自动重连(核心)
1. SSE 流断(弱网 / 切后台被 OS 杀连接)→ 客户端连接断,**后端回合继续跑**。
2. 网络恢复 / 回前台(connectivity_plus + WidgetsBindingObserver 监听)→ 自动 `GET .../turn/stream` resume。
3. `204` → 回合已结束 → 刷历史(`GET /sessions/{id}/messages`)拿最终结果;否则补播事件对齐 UI + 续接。

### 发送失败重试
- `POST turn/stream` **发起就失败**(网络不通、连不上)→ 标记该消息"发送失败" + **重试按钮**(手动重发)。
- 已发起后断 → 走上面的 resume(不是重发)。

### 关键区分(易错点)
- **发起失败 vs 中途断流**:发起失败(POST 没成功)→ 可重发;已发起后断(POST 成功、流中途断)→ 必须 **resume(GET)** 续看,**绝不能重发 POST**(会重复触发回合、双花)。
- 切后台短时不必立即重连;回前台统一 resume 一次最稳。
...
4. **聊天 + 回合流(核心)**:models + blocks 逻辑(移植 + 单测)+ SSE 解析 + 聊天页(历史重建 + live 渲染 + Composer)+ **断流重连(resume)+ 发送失败重试**。
