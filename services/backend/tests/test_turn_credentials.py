...
async def test_resolves_own_credential(session):
    ...
    api_key, base_url = await resolve_agent_key(session, str(cred.id), u.id, settings)
    assert api_key == "sk-real-key" and base_url == "https://x/v1"


async def test_empty_key_ref_returns_blank(session):
    settings = Settings(credential_key=_KEY_B64)
    assert await resolve_agent_key(session, "", uuid.uuid4(), settings) == ("", "")


async def test_invalid_key_ref_returns_blank(session):
    settings = Settings(credential_key=_KEY_B64)
    assert await resolve_agent_key(session, "not-a-uuid", uuid.uuid4(), settings) == ("", "")
...
async def test_foreign_credential_falls_back_to_blank(session):
    ...
    api_key, base_url = await resolve_agent_key(session, str(cred.id), other.id, settings)
