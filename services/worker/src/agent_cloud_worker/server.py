...
# 压缩器系统提示词(P3 结构化):摘要是后续回合的**唯一**历史上下文,泛化式一句话摘要会把
# 文件路径/报错原文/用户原话约束这类"下回合真正要用的锚点"抹掉。固定小节让模型逐项自查,
# 漏项直观可见(参考 Claude Code 压缩模板,按本产品裁剪)。
# 长度预算:摘要每次压缩都与旧摘要合并,无约束会单调增长——最终摘要本身挤占上下文窗口,
# 反过来加剧超窗(force_compact 只剩 1 条也救不了)。预算 + "优先精简更早内容"让它有损滚动。
# 小节顺序即截断优先级:万一输出触顶被 length 掐断,先丢的是**尾部**小节——把「当前状态/
# 未完成」这类续接锚点放前面,把「报错原文」这类篇幅大、可再查的放最后(审查中危修法①)。
_SUMMARIZE_SYSTEM = (
    "你是对话压缩器:把给定对话浓缩成结构化摘要,它将作为后续对话仅有的历史上下文。"
    "按下列小节输出(某节无内容就写「无」,不要编造):\n"
    "## 用户目标与诉求\n用户要做什么;明确说过的约束、偏好、纠正(尽量保留原话)。\n"
    "## 当前状态\n对话进行到哪一步,接下来最可能做什么。\n"
    "## 未完成事项\n待办、搁置的问题、用户提过但尚未处理的点。\n"
    "## 关键事实与决定\n已确认的事实、达成的决定、重要参数与结论。\n"
    "## 已产出成果\n创建/修改过的文件(保留完整路径)、执行过的关键命令、代码/文档要点。\n"
    "## 报错与修复\n遇到过的错误(保留报错关键原文)及最终处理结果。\n"
    "只输出摘要本身,不要额外解释。摘要总长控制在 1500 字以内;与已有摘要合并时"
    "优先精简更早的内容,越新的信息保留越多细节。"
)
# 摘要输出上限:1500 字中文≈1-1.6 字/token 本可 2048 兜住,但模板要求保留完整路径/报错
# 原文(ASCII 密集、按内容线性吃 token),留足余量到 3072 防触顶截断(审查中危修法②);
# 仍远小于模型窗口,防话痨模型无界输出(输入折叠段可达数万 token)的意义不变。
_SUMMARIZE_MAX_TOKENS = 3072
...
class WorkerServicer(worker_pb2_grpc.WorkerServicer):
    ...
    def _build_executor(self, channel: grpc.aio.Channel, request: worker_pb2.RunTurnRequest):
        ...
        # notify 不按 is_scheduled_run 关闭——定时任务到点提醒正是主用例。全平台暴露:
        # web 走浏览器通知,mobile 走 WS 推送长连(backend api/push.py,2026-07 起)。
